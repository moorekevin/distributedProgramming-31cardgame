...
public class Player {
	...
	private void createGame() throws InterruptedException {
		...
		if (game != null) {
			...
			lobbySpace.getAll(new ActualField("token"), new FormalField(String.class),
					new FormalField(String.class));
		}
	...
	private void knockOption() throws InterruptedException {
		...
		Boolean hasKnocked = (Boolean) lobbySpace.get(new ActualField("response"), new ActualField(id),
				new FormalField(Boolean.class))[2];
